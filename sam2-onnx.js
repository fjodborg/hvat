...
async function initOrt() {
    ...
    console.log('[SAM2] onnxruntime-web loaded, version:', ort.env.versions.common);
    console.log('[SAM2] WASM config: threads=%d, simd=%s', ort.env.wasm.numThreads, ort.env.wasm.simd);
}
...
async function createSession(modelData, name) {
    ...
    console.log('[SAM2] %s session created in %dms', name, Math.round(performance.now() - startTime));
    console.log('[SAM2] %s inputs: %s, outputs: %s', name, session.inputNames, session.outputNames);
        ...
        switch (type) {
            ...
            case 'result':
                ...
                if (pending) {
                    ...
                    pending.resolve(success ? { success: true, embeddings } : { success: false, error });
                }
                break;
        }
    };
...
async function loadModels() {
    ...
    try {
        ...
        encoderWorker.postMessage({ type: 'load', data: { encoderUrl: CONFIG.encoderUrl } });

        // Load decoder on main thread in parallel
        await initOrt();
        loadProgress = 0.5;

        let decoderData = await getCachedModel(CONFIG.decoderStore, CONFIG.decoderUrl);
        ...
        try {
            await decoderSession.run({
                image_embed: new ort.Tensor('float32', new Float32Array(256 * 64 * 64), [1, 256, 64, 64]),
                high_res_feats_0: new ort.Tensor('float32', new Float32Array(32 * 256 * 256), [1, 32, 256, 256]),
                high_res_feats_1: new ort.Tensor('float32', new Float32Array(64 * 128 * 128), [1, 64, 128, 128]),
            ...
            console.log('[SAM2] Decoder warmup done in %dms', Math.round(performance.now() - t0));
        } catch (e) {
...
async function runEncoder(imageData, width, height) {
    ...
    console.log('[SAM2] Encoding %dx%d image (request #%d)...', width, height, id);
...
async function runDecoder(embeddings, positivePoints, negativePoints, boundingBox) {
    if (!decoderSession) {
    ...
    try {
        ...
        console.log('[SAM2] Decoding: %d positive, %d negative points', positivePoints.length, negativePoints.length);
        const t0 = performance.now();

        // Prepare prompts
        const boxPoints = boundingBox ? 2 : 0;
        const actualPoints = positivePoints.length + negativePoints.length + boxPoints;
        ...
        const results = await decoderSession.run({
            image_embed: new ort.Tensor('float32', new Float32Array(embeddings.imageEmbed.data), embeddings.imageEmbed.shape),
            high_res_feats_0: new ort.Tensor('float32', new Float32Array(embeddings.highResFeats0.data), embeddings.highResFeats0.shape),
            high_res_feats_1: new ort.Tensor('float32', new Float32Array(embeddings.highResFeats1.data), embeddings.highResFeats1.shape),
        ...
        console.log('[SAM2] Decoder inference took %dms', Math.round(performance.now() - t0));

        // Extract best mask
        const masks = results.masks || results[decoderSession.outputNames[0]];
        const iouPreds = results.iou_predictions || results[decoderSession.outputNames[1]];
...
window.sam2 = {
    ...
    isReady: () => encoderWorker !== null && decoderSession !== null && isWarmedUp,
    runEncoder,
    runDecoder
};
