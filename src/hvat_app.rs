...
impl Application for HvatApp {
    ...
    fn view(&self) -> Element<'_, Self::Message> {
        ...
        // For Image Viewer tab, don't wrap in outer scrollable since it has its own
        // independent scrollables for left and right sidebars.
        // For other tabs, wrap in scrollable to support overflow content.
        let content_element: Element<'_, Message> = if self.current_tab == Tab::ImageViewer {
            // Image Viewer manages its own scrolling - just use content directly
            content
        } else {
            // Other tabs: wrap in scrollable for overflow support
            ...
            Element::new(scrollable_content)
        };
